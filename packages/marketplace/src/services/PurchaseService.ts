 ...
 * Integrates with GameMarketplace smart contract for instant 90/10 split.
 */
...
export class PurchaseService {
  ...
  constructor(config: PurchaseConfig) {
    ...
    this.compToken = new ethers.Contract(
      this.compTokenAddress,
      COMP_TOKEN_ABI,
      this.provider
    );
  }
  ...
  private async validatePurchase(
    ...
    if (BigInt(balance) < totalPrice) {
      return { valid: false, reason: 'Insufficient COMP balance' };
    }
  ...
  private async executeBlockchainPurchase(
    ...
    // 2. Approve the marketplace contract to spend COMP
  ...
  /**
   * Get COMP token balance for an address
   */
  async getBalance(address: string): Promise<string> {
    try {
      const balance = await this.compToken.balanceOf(address);
